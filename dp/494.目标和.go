...
func findTargetSumWays(nums []int, target int) int {
	...
	// 初始化dp表
	// dp[i][j]表示从前i个数字中选出若干个数字使得其和为j的方案数
	...
	for i := 1; i <= len(nums); i++ {
		for j := 0; j <= negSum; j++ {
			// 为什么会有两种情况？
			// 因为我们可以选择放nums[i-1]也可以选择不放nums[i-1]
			// 1. 如果选择放nums[i-1]，那么dp[i][j] = dp[i-1][j-nums[i-1]]
			// 2. 如果选择不放nums[i-1]，那么dp[i][j] = dp[i-1][j]

			// 为什么这里默认dp[i][j] = dp[i-1][j]？
			// 因为我们要求的是「和为j」的方案数，而不是「和等于j」的方案数
...
func findTargetSumWays(nums []int, target int) int {
	...
	// 此时将原问题转换为：nums 中存在几个子集 A，使得 A 中元素的和为 (target + sum(nums)) / 2
